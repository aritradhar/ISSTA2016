...
Hence, we see that the algorithm has worst case execution time bound of
...
the unoptimized algorithm may create $O(n)$ monitors, assign all of them to a set of objects,
 ...
 requiring all of the created monitors to be tracked for every event. therefore, the complexity of the
 unoptimized algorithm to handle $n$ events would be $O(n^2)$. Even though we expect this worst case to arise 
