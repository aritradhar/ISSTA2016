...
% being monitored 
especially when properties of interest are finite-state \note{what is finite 
state? Do we need to explain, or cite?} and are associated with objects 
...
event. \note{How to you ensure that the technique does not miss interested 
properties given that there is a limit on the number of monitors?} As a result, 
...
implementation~\xref{subsec:implementation} of our approach on realistic 
benchmarks and properties. Our evaluation~\xref{sec:results} shows 
\note{\todo{add stuff about the results.}}
