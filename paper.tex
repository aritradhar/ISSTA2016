...
and associated with collection objeccts. In this work, we propose a novel 
monitoring approach that investigates various trade-offs associated with
memory overhead of monitoring, the time boundedness of monitoring operations, and
the error reporting which are key attributes of resource-constrained systems.
Our approach is motivated by two observations which indicate that i) there is a
large redundancy among monitors in terms of their behaviors resulting in many
monitors detecting same errors, and ii) monitored events that belong to distinct monitors
are temporally separated. The monitor redundancy can be exploited to reduce the 
number of monitors, provide worst-case time bounds to monitoring operations, and 
improve the soundness of the optimized monitoring system. Our study based on a prototype 
implementation and using DaCapo benchmarks and some challenging combinations 
of finite state properties associated with collection objects indicates that the approach can 
effectively limit the memory overhead and monitoring operations time without compromising
with the error reporting and adding extra runtime overhead.
